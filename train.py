...
'''
            ...
            # else:
            src_test ='data/test.norm.tok.clean.tc.'+language_src
            src_test2 ='../description/bpe/test_2017_flickr.lc.norm.tok.bpe.'+language_src
            src_test3 ='../description/bpe/test_2017_mscoco.lc.norm.tok.bpe.'+language_src
            tgt_test ='data/test.norm.tok.clean.tc.'+language_tgt
            ...
            src_valid='data/val.norm.tok.clean.tc.'+language_src
            tgt_valid='data/val.norm.tok.clean.tc.'+language_tgt
...
def main():
    ...
    if opt.image_feat_type is not None:
        ...
        #加载图片特征
        """
        train_file_1 = tables.open_file(opt.img_train_feat_1, mode='r')
        img_train_feat_1 = train_file_1.root.local_feats[:]
        train_file_1.close()

        #train_file_2 = tables.open_file(opt.img_train_feat_2, mode='r')
        #img_train_feat_2 = train_file_2.root.local_feats[:]
        #train_file_2.close()

        #train_file_3 = tables.open_file(opt.img_train_feat_3, mode='r')
        #img_train_feat_3 = train_file_3.root.local_feats[:]
        #train_file_3.close()

        #train_file_4 = tables.open_file(opt.img_train_feat_4, mode='r')
        #img_train_feat_4 = train_file_4.root.local_feats[:]
        #train_file_4.close()

        #train_file_5 = tables.open_file(opt.img_train_feat_5, mode='r')
        #img_train_feat_5 = train_file_5.root.local_feats[:]
        #train_file_5.close()
        
        print("begin")
        #arr=([img_train_feat_1,img_train_feat_2,img_train_feat_3,img_train_feat_4,img_train_feat_5])
        train_img_feats = np.stack([img_train_feat_1,img_train_feat_1,img_train_feat_1,img_train_feat_1,img_train_feat_1],axis=1)
        #train_img_feats = np.stack([img_train_feat_1,img_train_feat_2,img_train_feat_3,img_train_feat_4,img_train_feat_5],axis=1)
        print("end")


        valid_file_1 = tables.open_file(opt.img_valid_feat_1, mode='r')
        img_valid_feat_1 = valid_file_1.root.local_feats[:]
        valid_file_1.close()

        valid_file_2 = tables.open_file(opt.img_valid_feat_2, mode='r')
        img_valid_feat_2 = valid_file_2.root.local_feats[:]
        valid_file_2.close()

        valid_file_3 = tables.open_file(opt.img_valid_feat_3, mode='r')
        img_valid_feat_3 = valid_file_3.root.local_feats[:]
        valid_file_3.close()

        valid_file_4 = tables.open_file(opt.img_valid_feat_4, mode='r')
        img_valid_feat_4 = valid_file_4.root.local_feats[:]
        valid_file_4.close()
 
        valid_file_5 = tables.open_file(opt.img_valid_feat_5, mode='r')
        img_valid_feat_5 = valid_file_5.root.local_feats[:]
        valid_file_5.close()

        valid_img_feats = np.stack([img_valid_feat_1,img_valid_feat_2,img_valid_feat_3,img_valid_feat_4,img_valid_feat_5],axis=1)

        """

        train_file_1 = tables.open_file(opt.img_train_feat_1, mode='r')
        train_img_feats = train_file_1.root.local_feats[:]
        train_file_1.close()
        valid_file_1 = tables.open_file(opt.img_valid_feat_1, mode='r')
        valid_img_feats = valid_file_1.root.local_feats[:]
        valid_file_1.close()
        print("1")
        ...
        if image_num > 1:
            train_file_2 = tables.open_file(opt.img_train_feat_2, mode='r')
            img_train_feat_1 = train_file_2.root.local_feats[:]
            train_img_feats = np.concatenate((train_img_feats,img_train_feat_1),axis=1)
            train_file_2.close()

            valid_file_2 = tables.open_file(opt.img_valid_feat_2, mode='r')
            img_valid_feat_1 = valid_file_2.root.local_feats[:]
            valid_img_feats = np.concatenate((valid_img_feats, img_valid_feat_1),axis=1)
            valid_file_2.close()
            print("2")

        if image_num > 2:
            train_file_3 = tables.open_file(opt.img_train_feat_3, mode='r')
            img_train_feat_1 = train_file_3.root.local_feats[:]
            train_img_feats = np.concatenate((train_img_feats,img_train_feat_1),axis=1)
            train_file_3.close()
            valid_file_3 = tables.open_file(opt.img_valid_feat_3, mode='r')
            img_valid_feat_1 = valid_file_3.root.local_feats[:]
            valid_img_feats = np.concatenate((valid_img_feats, img_valid_feat_1),axis=1)
            valid_file_3.close()
            print("3")
        ...
        if image_num > 3:
            train_file_4 = tables.open_file(opt.img_train_feat_4, mode='r')
            img_train_feat_1 = train_file_4.root.local_feats[:]
            train_img_feats = np.concatenate((train_img_feats,img_train_feat_1),axis=1)
            train_file_4.close()
            valid_file_4 = tables.open_file(opt.img_valid_feat_4, mode='r')
            img_valid_feat_1 = valid_file_4.root.local_feats[:]
            valid_img_feats = np.concatenate((valid_img_feats, img_valid_feat_1),axis=1)
            valid_file_4.close()
            print("4")
        ...
        if image_num > 4:
            train_file_5 = tables.open_file(opt.img_train_feat_5, mode='r')
            img_train_feat_1 = train_file_5.root.local_feats[:]
            train_img_feats = np.concatenate((train_img_feats,img_train_feat_1),axis=1)
            train_file_5.close()
            valid_file_5 = tables.open_file(opt.img_valid_feat_5, mode='r')
            img_valid_feat_1 = valid_file_5.root.local_feats[:]
            valid_img_feats = np.concatenate((valid_img_feats, img_valid_feat_1),axis=1)
            valid_file_5.close()
            print("5")
        ...
        print('train',train_img_feats.shape)
        print('valid',valid_img_feats.shape)
