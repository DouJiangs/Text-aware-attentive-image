...
class RNNDecoderBase(nn.Module):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
        ...
        self.l = nn.Linear(hidden_size*196,hidden_size)
        self.ll = nn.Linear(196*5,196)
        self.sigmoid = nn.Sigmoid()
        self.sigmoid_linear = nn.Linear(196*512*5,1)
        
        self.gate = nn.Linear(hidden_size,1)
        self.img_5_attention = onmt.modules.GlobalAttention(
             hidden_size,coverage=coverage_attn,
             attn_type='dot', gpu=gpu, multi_query=False
        )
    ...
    def forward(self, tgt, memory_bank, state, memory_lengths=None, context_img=None):
        ...
        # context_img_5 = [batch, 196, 5, hidden]
        #context_img_5 = context_img.transpose(1,2).contiguous()
        # context_img_5 = [batch, 5, 196, hidden]

        #context_img_5 = context_img_5.view(tgt_batch,5, self.hidden_size*196)
        # context_img_5 = [batch, 5, 196*hidden]

        #context_img_5 = self.l(context_img_5)
        # context_img_5 = [batch, 5, hidden]

        #img5 = self.img_5_attention(context_img_5.contiguous(),memory_bank,memory_lengths)
        # context_img_5 = [batch, 5, hidden]
        # memory_bank = [batch, src_len, hidden]
        # img5 = [batch, 5, src_len]

        #img5 = self.sm(img5)
        # softmax

        #img5 = torch.bmm(img5,memory_bank)
        # img5 = [batch, 5, src_len]
        # memory_bank = [batch, src_len, hidden]
        # img5 = [batch, 5, hidden]

        #img5 = img5.unsqueeze(1)
        # img5 = [batch, 1, 5, hidden]

        #img5 = img5.repeat(1,196,1,1)
        # img5 = [batch,196,5,hidden]

        #context_img = context_img + img5
        # img5 = [batch,196,5,hidden]
        # context_img = [batch,196,5,hidden]

        #context_img = context_img.view(tgt_batch,-1,self.hidden_size).contiguous()
        # context_img = [batch,196*5,hidden]


        if context_img is not None and self.bi_attention:
            '''context_img_5 = context_img.transpose(1,2).contiguous()
            context_img_5 = context_img_5.view(tgt_batch,-1, self.hidden_size*196)
            context_img_5 = self.l(context_img_5)
            img5 = self.img_5_attention(context_img_5.contiguous(),memory_bank,memory_lengths)
            img5 = self.SM(img5)
            img5 = torch.bmm(img5,memory_bank)
            img5 = img5.unsqueeze(1)
            img5 = img5.repeat(1,196,1,1)
            context_img = 0.4*context_img + 0.6*img5
            context_img = context_img.reshape(tgt_batch,-1,self.hidden_size).contiguous()
            '''

            image = context_img
            text = memory_bank
            #image = image.transpose(1,2)
            image_atten = image.transpose(2,3)
            text = text[:,[-1] ,:]
            text = text.unsqueeze(1)
            text = text.repeat(1, 196, 1, 1)
            attention = torch.matmul(text,image_atten)
            a = self.my_softmax(attention)
            ...
            #gate = gate.view(tgt_batch,1,1,1)
            #gate = gate.expand(tgt_batch,196,5,512)
            #context_img = (1-gate)*context_img + gate*img5 
            #context_img = 0.4*context_img + 0.6*img5
            #context_img = context_img.reshape(tgt_batch,-1,self.hidden_size).contiguous()
            #context_img = context_img.reshape(tgt_batch,self.hidden_size,-1).contiguous()
            #context_img = self.ll(context_img)
            #context_img = context_img.reshape(tgt_batch,196,self.hidden_size)
            ...
            if memory_lengths is not None:
                ...
                img_text_alignment2.data.masked_fill_(~mask, -float('inf'))
...
class ImageLocalFeaturesProjector(nn.Module):
    ...
    def __init__(self, num_layers, nfeats, outdim, dropout,
            use_nonlinear_projection,image_num):
        """
        Args:
            num_layers (int): 1.
        ...
        layers = []
        # reshape input
        layers.append( View(-1,14*14,image_num,nfeats) )
