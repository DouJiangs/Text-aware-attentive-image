...
def make_decoder(opt, embeddings):
    ...
    if opt.decoder_type == "transformer":
        ...
        return InputFeedRNNDecoder(opt.rnn_type, opt.brnn,
                                   ...
                                   bi_dim_attn=opt.bi_attention,
                                   co_attention=opt.co_attention)
    else:
...
def make_encoder_image_local_features(opt):
    ...
    # Validate that these values work for other CNN architectures as well.
    if 'vgg' in opt.img_train_feat_1.lower():
        feat_size = 512
    else:
        feat_size = 512
    num_layers = 1
    return ImageLocalFeaturesProjector(num_layers, feat_size, opt.rnn_size,
            opt.dropout_imgs, opt.use_nonlinear_projection,opt.image_num)
